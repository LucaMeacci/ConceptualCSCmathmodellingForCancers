...
rho=[mh*0.1, mh*0.1, mh*0.1]; %replication
rho1=[mh*0.06, mh*0.05, mh*0.05]; 
rho2=[mh*0.08, mh*0.07, mh*0.07]; 
rho3=[mh*0.09, mh*0.08, mh*0.08]; 
rho4=[mh*0.04, mh*0.03, mh*0.03]; 

d=0.8; %CSC -> CC (1)

fm=1.0;
mu1=[fm*mh*0.009, fm*mh*0.010, fm*mh*0.010]; %mortality (1)
mu2=[fm*mh*0.009, fm*mh*0.010, fm*mh*0.010]; %mortality (2)
mu3=[fm*mh*0.009, fm*mh*0.010, fm*mh*0.010]; %mortality (3) 
mu4=[fm*mh*0.014, fm*mh*0.015, fm*mh*0.015]; %mortality (4)
...
if size(MCSC,1)>0
plot3(MCSC(:,1),MCSC(:,2),MCSC(:,3),'.k','MarkerSize',10);
end
...
LL1z=(LL1yz-LL1y)/n+1;
plot3(LL1x,LL1y,LL1z,'.m','MarkerSize',10)
%
[LL2x,LL2yz]=find((~(L-2)));
LL2y=mod((LL2yz-1),n)+1;
LL2z=(LL2yz-LL2y)/n+1;
plot3(LL2x,LL2y,LL2z,'.c','MarkerSize',10)
%
[LL3x,LL3yz]=find((~(L-3)));
LL3y=mod((LL3yz-1),n)+1;
LL3z=(LL3yz-LL3y)/n+1;
plot3(LL3x,LL3y,LL3z,'.','color',[0.9100    0.4100    0.1700],'MarkerSize',10);
hold off
pause
end
    ...
    for i=1:n    
        for j=1:n
            ...
            elseif G_prec(i,j,l)==0.7 %CC1 case
                ...
                if rand<mu2(L(i,j,l)) %apoptosis condition
                    G(i,j,l)=2; %CC1 dies
                    L(i,j,l)=0;
                else
            ...
            elseif G_prec(i,j,l)==1.3 %CC3 case
                if rand<mu3(L(i,j,l)) %apoptosis condition
                    G(i,j,l)=2; %CC3 dies
                    L(i,j,l)=0;
                else
            ...
            elseif G_prec(i,j,l)==1.6 %CC4 case
                if rand<mu4(L(i,j,l)) %apoptosis condition
                    G(i,j,l)=2; %CC1 dies
                    L(i,j,l)=0;
                else
...
hold on
if size(MCC1,1)>0
plot3(MCC1(:,1),MCC1(:,2),MCC1(:,3),'.b','MarkerSize',10);
end
if size(MCC2,1)>0
plot3(MCC2(:,1),MCC2(:,2),MCC2(:,3),'.g','MarkerSize',10);
end
if size(MCC3,1)>0
plot3(MCC3(:,1),MCC3(:,2),MCC3(:,3),'.y','MarkerSize',10);
end
if size(MCC4,1)>0
plot3(MCC4(:,1),MCC4(:,2),MCC4(:,3),'.r','MarkerSize',10);
end
if size(MCSC,1)>0
plot3(MCSC(:,1),MCSC(:,2),MCSC(:,3),'.k','MarkerSize',10);
end
